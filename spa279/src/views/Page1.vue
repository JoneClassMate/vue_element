  ...
  <div class="hello">
    <div>
      <h1>{{msg}}</h1>
        <h1>{{namse}}</h1>
        <div>
            <el-button type="success" @click="doClick">mutations同步事件</el-button>
            <el-button type="success" @click="doClick1">actions异步事件</el-button>
        </div>
    </div>
  </div>
</template>
...
<script>
  export default {
    ...
    methods:{
      doClick:function(){
        //参数1:要改变的事件名 参数2:改变值过后的值（注！json类型键值对）
        this.$store.commit('setName',{name:"北京欢迎你"})
      },
      doClick1:function(){
        this.$store.dispatch('setResturantNameByAsync',{name:'怀化欢迎你'})
      }
    }
  }
</script>
