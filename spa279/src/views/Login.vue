    ...
    <div class="login-wrap">
      <el-form class="login-container" :model="ruleForm" :rules="rules" ref="ruleForm">
        <div style="text-align: center;">
          <h1>{{ msg }}{{$store.getters.getRoleId}}</h1>
        </div>
        ...
        <el-link @click="doReg()" type="info">我要注册</el-link>
        <el-link @click="doFor()" style="margin-left: 220px;" type="info">忘记密码?</el-link>
      </el-form>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      doSubmit: function() {
       ...
       /* console.info("路径==="+url) */
        /*        axios.get(url,{//请求地址：请求参数
                  params:data
                }).then(function(resp){//成功回调函数
                  console.info(resp)
                }).catch(function(resp){//失败回调函数
                  console.info(resp)
                }); */
        ...
        this.$refs['ruleForm'].validate((valid) => {
          if (valid) { //验证成功
            //开始向后端请求
            this.axios.post(url, data //请求地址：请求参数
            ).then((resp) => { //成功回调函数
              console.info(resp)//打印成功回调函数
              //判断为true就跳转组件
              if (resp.data.success) {
                //将用户编号保存到vuex中
                this.$store.commit('setRoleId',{roleId:resp.data.rows})
                //弹窗提示
                this.$message({
