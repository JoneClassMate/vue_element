  ...
  <div class="hello">
    <div class="login-wrap">
      <el-form class="login-container" :model="ruleForm" :rules="rules" ref="ruleForm">
        ...
        <el-link @click="doLogin()" type="info">快速登录</el-link>
      </el-form>
    </div>
  </div>
</template>
  ...
  export default {
    data() {
      ...
      var validatePass = (rule, value, callback) => {
        ...
        } else if(!va.test(value)){
          callback(new Error('密码必须为2-6位的数字'));
        }else{
          callback();
        }
      };
      ...
      var nameV = (rule, value, callback) => {
       ...
       if(value===''){
         ...
         callback(new Error('账号必须为2-6位的字母'));
       }else{
         callback()
       }
